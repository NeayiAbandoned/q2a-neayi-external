...
function qa_get_login_links($relative_url_prefix, $redirect_back_to_url)
{
	...
	return array(
		'login' => 'http://pratiques.dev.tripleperformance.fr/test_qa_external_user/login.php?redirect='.urlencode(qa_opt('site_url').$redirect_back_to_url),
		'register' => 'http://pratiques.dev.tripleperformance.fr/test_qa_external_user/register.php?redirect='.urlencode(qa_opt('site_url').$redirect_back_to_url),
		'logout' => 'http://pratiques.dev.tripleperformance.fr/test_qa_external_user/logout.php?redirect='.urlencode(qa_opt('site_url').$redirect_back_to_url)
	);
...
function qa_get_logged_in_user()
{
	...
	*/
	if (isset($_COOKIE['external_login_session_id']))
	{
		$sessionMap = array();
		$sessionMap[1] = 1; // Bertrand
		$sessionMap[2] = 9; // Henri
		$sessionMap[3] = 213; // Jean-François

		if (isset($sessionMap[$_COOKIE['external_login_session_id']]))
		{
			$result = qa_db_read_one_assoc(qa_db_query_sub(
				'SELECT userid, handle, email, level FROM qa_users WHERE userid=#',
				$sessionMap[$_COOKIE['external_login_session_id']]
			));
			...
			if (is_array($result)) {
				return array(
					...
					'email' => $result['email'],
					'level' => $result['level'] ? QA_USER_LEVEL_ADMIN : QA_USER_LEVEL_BASIC
				);
			}
		}
	}
	...
	*/
	...
	if (count($publicusernames)) {
		...
		$results = qa_db_read_all_assoc(qa_db_query_raw(
			'SELECT handle, userid FROM qa_users WHERE handle IN (' . implode(',', $escapedusernames) . ')'
		));

		foreach ($results as $result)
			$publictouserid[$result['handle']] = $result['userid'];
	}
...
function qa_get_public_from_userids($userids)
{
	...
	*/
	...
	if (count($userids)) {
		...
		$results = qa_db_read_all_assoc(qa_db_query_raw(
			'SELECT handle, userid FROM qa_users WHERE userid IN (' . implode(',', $escapeduserids) . ')'
		));

		foreach ($results as $result)
			$useridtopublic[$result['userid']] = $result['handle'];
	}
